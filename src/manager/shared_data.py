...
class SharedData:
    ...
    def delete(self, category, key):
        if self._data.get(category) is None:
            return
        self._lock.acquire()
    ...
    def run(self, sharedData):
        sharedData = sharedData.getDataObj()
        ...
        while True:
            try:
                ...
                while True:
                    ...
                    if tickTime > self._collectPeriod:
                        tickTime = self._collectPeriod
                    time.sleep(self._collectPeriod - tickTime)
            except Exception as e:
                systemAlerter.sendAlert('shared data abort', traceback.format_exc())
    ...
    def init(self, sharedData):
        self._initData(sharedData)
        return sharedData

    def _initData(self, sharedData):
        metaDb = SoulDBCon.getHotDB('meta')
        alertDb = SoulDBCon.getHotDB('alert')
        self._updateData(metaDb, alertDb, sharedData)
    ...
    def _loadPowerStations(self, metaDb, sharedData):
        coll = metaDb['powerStations']
        powerStations = coll.find({'syncedAt': {'$exists': False}}, {'_id': False, 'mcno': True, 'psStatusCode': True, 'psName': True, 'manage.onm': True})
        if powerStations:
