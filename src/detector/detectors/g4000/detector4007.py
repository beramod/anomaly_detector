...
## ROOM Fault Temp 
class Detector4007(BaseDetectorForTemp):
    _eventCode = 4007
    ...
    def _process(self):
        ...
        batRoomTempSettings = self._getRoomTempSettings('BAT')
        if batRoomTempSettings is None or len(batRoomTempSettings) < 1:
            return False

        faultTempSettingMap = self._getRoomFaultSettingsMap(batRoomTempSettings)
        if faultTempSettingMap is None or faultTempSettingMap == {}:
            return False

        # mode : None or all(전체 장비에 동일 기준 적용) or units(장비별 개별 적용)
        mode = self._getFaultCheckMode(faultTempSettingMap) 

        if mode is None:
            return False
        
        event = self._getFaultEvent(data, faultTempSettingMap, mode)
    ...
    def _getFaultEvent(self, data, faultSettingMap, mode):
        event = None
        faultInfos = []

        for each in data.get('tempHumiditySensors'):
            if each.get('room') != 'BAT':
                continue
            sensorno = int(each.get('sensorno'))
            temp = each.get('temp')
            tempLimitMap = faultSettingMap.get(0).get('fault') if mode == 'all' else faultSettingMap.get(sensorno).get('fault')
